...
class LlavaMetaModel:
    ...
    def load_clip_to_cpu(self,backbone_name, design_details):
    ...
    def get_prompt(self, batch_size: int,device): 
        ...
        prompt_tokens= torch.arange(prompt_config['num_virtual_tokens'] * 
                                    prompt_config['num_transformer_submodules']).long()
        prompt_tokens = (
        ...
        if prompt_config['inference_mode']:
            ...
            #print("propmt tokens: ",prompt_tokens)
            prompts = self.prompt_encoder(prompt_tokens)
            #print("propmts: ",prompts)
        return prompts
...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, attention_mask, 
        ...
        if any(x.shape != new_input_embeds[0].shape for x in new_input_embeds):
            ...
            if self.prompt_config['is_prompt_learning']:
                print("new label 1:",new_labels.shape)
                ...
                if new_labels is not None:
                    ...
                    print("new label 1 with prompt:",new_labels.shape)
            if attention_mask is not None:
                new_attention_mask = []
