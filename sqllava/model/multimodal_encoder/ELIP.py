...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, adptr = False):
        super().__init__()
        self.width = width
        self.layers = layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask,adptr) for _ in range(layers)])
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int,adptr):
        super().__init__()
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image):
        #print("default weight type",self.dtype)
...
def build_model(state_dict: dict,config):
...
def load_elip(name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", jit: bool = False, download_root: str = None, config=None):
...
def ELIP_model(pretrain_clip='',pretrained='',device='cpu',config=None):
